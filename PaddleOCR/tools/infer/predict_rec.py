...
def main(args):
    ...
    for ino in range(len(img_list)):
        results[valid_image_file_list[ino]] = rec_res[ino][0]
        logger.info(
    ...
    for key in expected_results.keys():
        if results[key] == expected_results[key]:
            count += 1
        else:
            new_result = ""
            for i in range(len(results[key])):
                masked = results[key][:i] + "[MASK]"+ results[key][i+1:]
                print(masked)
            ...
            print(new_result)
            if new_result == expected_results[key]:
                count +=1
                print(key)
